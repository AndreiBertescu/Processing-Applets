...
String geomPath = "Topo Custom 16x8.tiff"; // height map path - Topo Custom 16x8.tiff - lroc_color_poles_16kv2.tiff - 5672_mars_6k_topo.jpg
...
void setup() {
  ...
  if (geom) {
    //load height map
    img = loadImage(geomPath);
    img.loadPixels();
    resolution = new PVector(img.width, img.height);
    //load color map
    cimg = loadImage(colorPath);
    cimg.loadPixels();
